...
const DOWNNLOAD_FOLDER = "/Users/shashankshivakumar/Desktop/bot_demo/public"
...
function ChatbotApp() {
  ...
  const generateDownloadLink = async (filePath, linkText = "Download File") => {
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        reader.onload = () => {
          ...
          const href = `data:${mimeType};base64,${base64File.split(",")[1]}`; // Create Base64 link
          resolve(href); // Resolve with href
      ...
      throw new Error(`Error generating download link: ${error.message}`);
    }
  };
  ...
  const renderBotMessage = (message, index) => {
    ...
    if (message.custom) {
        ...
        if (payload.type === "download_file") {
          ...
          generateDownloadLink(filePath, "Download File")
            .then((downloadLink) => {
                console.log("DOWNLOAD LINK", downloadLink)
              // Render the download link in the UI
              return (
                console.log("DOWNLOAD LINKKKKKK"),
                <div key={index} style={{ margin: "10px 0" }}>
                  <strong style={{ color: "#00C853" }}>Bot:</strong>{" "}
                  <span dangerouslySetInnerHTML={{ __html: downloadLink }}></span>
                </div>
              );
            })
            .catch((error) => {
              console.error("Failed to create the download link:", error);
            });
        }
